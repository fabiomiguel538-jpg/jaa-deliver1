...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ 
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex font-sans">
      ...
      {selectedStore && (<div className="fixed inset-0 bg-black/60 backdrop-blur-md flex items-center justify-center z-[100] p-4"><div className="bg-white w-full max-w-4xl rounded-[3rem] p-8 shadow-2xl relative border-4 border-white max-h-[90vh] overflow-y-auto animate-in zoom-in-95"><button onClick={() => setSelectedStoreId(null)} className="absolute top-6 right-6 text-gray-300 text-xl font-bold">✕</button><h2 className="text-2xl font-black text-gray-800 mb-6 font-jaa italic flex items-center gap-3">{Icons.store} Dados da Loja</h2><div className="space-y-6"><div className="grid grid-cols-1 md:grid-cols-2 gap-4"><InfoCard label="Nome Fantasia" value={selectedStore.name} icon="🏪" /><InfoCard label="CNPJ" value={selectedStore.taxId} icon="🆔" /><InfoCard label="Email" value={selectedStore.email} icon={Icons.email} /><InfoCard label="WhatsApp" value={selectedStore.whatsapp} icon={Icons.whatsapp} /><InfoCard label="Endereço" value={selectedStore.address} icon="📍" /><InfoCard label="Status" value={selectedStore.status} icon="🚦" /><InfoCard label="Saldo Carteira" value={`R$ ${selectedStore.balance.toFixed(2)}`} icon={Icons.wallet} /></div><div className="bg-orange-50 p-6 rounded-[2rem] border border-orange-100 space-y-4">
